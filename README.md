...
[![.NET](https://github.com/Kudaraidee/miningcore/actions/workflows/dotnet.yml/badge.svg)](https://github.com/blackmennewstyle/miningcore/actions/workflows/dotnet.yml)
...
### Ubuntu 26.04 (recommended)

Builds inside Docker, so no toolchain has to be installed on the host. A running
Docker daemon is the only requirement:

```console
./build-ubuntu-26.04.sh
```

The artifacts are written to `./build` (pass a different path as the first
argument). To produce a runnable image instead:
...
These install the toolchain on the host. Miningcore targets .NET 10, which these
releases do not carry in their own archive, so both scripts pull the SDK from the
...
Scripts for Ubuntu 20.04, Ubuntu 21.04 and Debian 11 were removed - all three are
past end-of-life. On any other distribution use the Docker build described under
[Building using Docker Engine](#building-using-docker-engine).
...
```console
docker run --rm -v $(pwd):/app -w /app mcr.microsoft.com/dotnet/sdk:10.0 /bin/bash -c 'apt update && apt install cmake ninja-build build-essential libssl-dev pkg-config libboost-all-dev libsodium-dev libzmq5n libzmq3-dev golang-go libgmp-dev -y --no-install-recommends && cd src/Miningcore && dotnet publish -c Release -o /app/build/'
```
...
```console
docker run --rm -v $(pwd):/app -w /app mcr.microsoft.com/dotnet/sdk:10.0 /bin/bash -c 'apt update && apt install cmake ninja-build build-essential libssl-dev pkg-config libboost-all-dev libsodium-dev libzmq5n libzmq3-dev golang-go libgmp-dev -y --no-install-recommends && cd src/Miningcore && dotnet publish -c Release -o /app/build/ -r osx-x64 --self-contained false'
```
...
```console
docker run --rm -v $(pwd):/app -w /app mcr.microsoft.com/dotnet/sdk:10.0 /bin/bash -c 'apt update && apt install cmake ninja-build build-essential libssl-dev pkg-config libboost-all-dev libsodium-dev libzmq5n libzmq3-dev golang-go libgmp-dev -y --no-install-recommends && cd src/Miningcore && dotnet publish -c Release -o /app/build/ -r win-x64 --self-contained false'
```
